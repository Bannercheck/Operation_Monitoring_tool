...
const S: Record<string, [string, string]> = {
  ...
  // ops details
  ops_scope_env: ["Ortam", "Environment"], ops_scope_host: ["Sunucu", "Host"], ops_scope_reset: ["Kapsamı sıfırla", "Reset scope"], ops_multi: ["{n} sunucu birlikte (metrikler ortalanır)", "{n} hosts together (metrics averaged)"], ops_all: ["tümü", "all"],
  ops_detail: ["Detay", "Detail"], ops_connect: ["Sunucu bağla", "Connect a server"], od_window: ["canlı · son 15 dk", "live · last 15 min"], od_rate: ["Dakikalık olaylar", "Events per minute"], od_errors: ["Son ERROR+ olayları", "Latest ERROR+ events"],
  od_availability: ["Erişilebilirlik", "Availability"], od_sla: ["SLA", "SLA"], od_budget: ["Hata bütçesi", "Error budget"], od_p95: ["p95 gecikme", "p95 latency"], od_cpu: ["CPU (sunucu bazında)", "CPU by host"], od_memory: ["Bellek (sunucu bazında)", "Memory by host"], od_disk: ["Disk (sunucu bazında)", "Disk by host"], od_gpu: ["GPU (sunucu bazında)", "GPU by host"], od_files: ["Log dosyaları", "Log files"],
  od_latest: ["son", "latest"], od_avg: ["ort", "avg"], od_max: ["maks", "max"], od_threshold: ["eşik", "threshold"], od_breaches: ["Eşik aşımları", "Breaches"], od_minute_avail: ["Dakikalık erişilebilirlik", "Availability per minute"], od_burn: ["Hata bütçesi tüketimi", "Error budget burn"],
  od_err_by_service: ["Servise göre ERROR+", "ERROR+ by service"], od_err_by_host: ["Sunucuya göre ERROR+", "ERROR+ by host"], od_what_lowers: ["Rakamı düşüren örüntüler", "What lowers the figure"], od_slowest: ["En yavaş olaylar", "Slowest events"], od_samples: ["örnek", "samples"],
  od_target: ["hedef", "target"], od_allowed: ["izinli hata", "allowed errors"], od_breach_min: ["ihlal dakikası", "breach minutes"], od_discovery: ["Ajanların keşfettiği uygulamalar", "Applications discovered by agents"], od_open_file: ["Dosyayı aç", "Open file"], files_export: ["Satırları indir", "Download lines"],
  ops_tickets: ["En ilgili ticket'lar", "Most relevant tickets"], ops_env_summary: ["Ortamlar", "Environments"], ops_files_tile: ["hatalı dosya", "files with errors"],
  // ops extras
  ...
  // incident extras
  fc_what: ["Ne oldu", "What"], fc_why: ["Neden bu kök neden", "Why this root cause"], fc_where: ["Nerede", "Where"], fc_when: ["Ne zaman", "When"], fc_resolved: ["Düzelme", "Resolution"], fc_todo: ["İlk aksiyon", "First action"], fc_owner: ["sorumlu", "owner"], fc_chain: ["zincir", "chain"], fc_alts: ["Değerlendirilen alternatifler", "Alternatives considered"],
  fc_group: ["Fırtına grubu", "Storm group"], fc_map: ["Haritada göster", "Show on map"], pb_seen_before: ["Daha önce görüldü", "Seen before"], pb_times: ["{n} kez, {d} veri setinde", "{n} times in {d} datasets"],
  create_action: ["Aksiyon oluştur", "Create action"], custom_action: ["Özel aksiyon", "Custom action"], llm_prompt: ["LLM istemi", "LLM prompt"], llm_explain: ["LLM ile açıkla", "Explain with the LLM"], llm_answer: ["LLM cevabı", "LLM answer"],
  fb_title: ["Kart doğru mu?", "Is the card right?"], fb_up: ["Doğru", "Right"], fb_down: ["Yanlış", "Wrong"], fb_correct: ["Doğru kök neden", "Correct root cause"], fb_keep: ["Gösterilen doğru", "As shown"], fb_noise: ["Bu gürültü, incident değil", "This is noise, not an incident"], fb_other: ["Başka (yorumda)", "Other (in the comment)"],
  fb_comment: ["Yorum", "Comment"], fb_send: ["Gönder", "Send"], fb_saved: ["Kaydedildi · {n} kural önerisi oluştu", "Saved · {n} rule proposals created"], kb_related: ["Bilgi tabanında benzer dersler", "Related lessons"], kb_related_none: ["Benzer ders yok.", "No related lesson yet."],
  noise_heat: ["Fırtına ısı haritası (servis × zaman)", "Storm heat map (service × time)"], noise_low_groups: ["Kart olmayan küçük gruplar", "Small groups that did not become cards"], noise_reason: ["Neden", "Reason"], tab_actions_ds: ["Bu setin aksiyonları", "Actions of this dataset"],
  ds_samples: ["Örnek veri setleri", "Sample datasets"], ds_fetch: ["Uzak kaynaktan çek", "Fetch from a remote source"], ds_fetch_kind: ["Tür", "Kind"], ds_fetch_url: ["Adres", "URL"], ds_fetch_headers: ["Başlıklar (satır başına Key: Value)", "Headers (Key: Value per line)"], ds_fetch_path: ["JSON yolu (isteğe bağlı)", "JSON path (optional)"],
  ds_fetch_tool: ["MCP aracı", "MCP tool"], ds_fetch_tools: ["Araçları listele", "List tools"], ds_fetch_go: ["Çek ve yükle", "Fetch and load"], ds_mapping: ["Sütun eşlemesi (JSON, isteğe bağlı)", "Column mapping (JSON, optional)"],
  kb_doc_upload: ["Belge yükle (txt / md)", "Upload a document (txt / md)"], kb_propose_llm: ["LLM'den kural önerisi iste", "Ask the LLM for rules"],
  sys_maint: ["Bakım", "Maintenance"], sys_clear_live: ["Canlı tamponu temizle", "Clear the live buffer"], sys_prune: ["Eski anlık görüntüleri sil (10 kalsın)", "Prune snapshots (keep 10)"], sys_zip_deploy: ["Zip ile sürüm yükle", "Deploy a release zip"], sys_git_update: ["Git ile güncelle", "Update from git"],
  nav_readme: ["README", "README"],
  // datasets extras
