...
export default function Ops() {
  ...
  return (
    <div className="stack">
      ...
      {!!m?.summary && <div className="grid k4" style={{ gridTemplateColumns: "repeat(5, minmax(0, 1fr))" }}>
        {["cpu", "memory", "disk", "gpu"].map((k, i) => { const sm = m.summary[k] ?? {}; const worst = sm.max != null && sm.max >= THR[k]; return <div key={k} {...clickable(k as Kind)}><div className="card kpi" style={{ ["--acc" as any]: worst ? "#f87171" : PALETTE[i], padding: "10px 12px" }}><div className="lb">{k}</div><div className="row between"><div className="v" style={{ fontSize: 22 }}>{sm.avg != null ? `${Math.round(sm.avg)}%` : "-"}</div><div style={{ width: 120 }}><Spark data={metricSeries(k)} color={worst ? "#f87171" : PALETTE[i]} /></div></div><div className="sub">{sm.max != null ? `${t("od_max")} ${Math.round(sm.max)}% · ${sm.worst ?? ""} · ${sm.hosts} ${t("host").toLowerCase()}` : "-"}</div></div></div>; })}
        <div {...clickable("files")}><div className="card kpi" style={{ ["--acc" as any]: badFiles ? "#fb923c" : "#8b98ad", padding: "10px 12px" }}><div className="lb">{t("od_files")}</div><div className="v" style={{ fontSize: 22 }}>{files.data?.length ? `${badFiles}/${files.data.length}` : "-"}</div><div className="sub">{t("ops_files_tile")} · {t("ops_detail")} ▾</div></div></div>
      </div>}
