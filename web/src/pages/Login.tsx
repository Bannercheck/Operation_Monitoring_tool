...
export default function Login() {
  ...
  const options = useQuery({ queryKey: ["auth-options"], queryFn: () => get<{ register: boolean; verify: boolean }>("/auth/options") });
  ...
  return (
    <div className="login-bg"><div className="login">
      ...
      {!challenge && mode === "login" && (<><div className="or">{t("login_or")}</div><div className="sso">{(providers.data ?? [{ name: "google", label: "Google", configured: false }, { name: "microsoft", label: "Microsoft", configured: false }]).filter((p) => p.name !== "oidc" || p.configured).map((p) => (
