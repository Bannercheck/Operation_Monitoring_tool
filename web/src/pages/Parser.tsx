...
type Tab = "lib" | "test" | "unparsed" | "base";
...
export default function ParserPage() {
  ...
  return <div className="stack">
    ...
    <Tabs value={tab} onChange={setTab} tabs={[{ k: "lib", label: t("gk_tab_lib") }, { k: "test", label: t("gk_tab_test") }, { k: "unparsed", label: t("gk_tab_unparsed") }, { k: "base", label: t("gk_tab_base") }]} />
    {tab === "lib" && <Card right={editable ? <Btn kind="primary" sm onClick={() => setEdit({ ...EMPTY })}>➕ {t("gk_add")}</Btn> : undefined}>
      <div className="row" style={{ marginBottom: 8 }}><input className="input grow" placeholder={t("sys_tbl_filter")} value={q} onChange={(e) => setQ(e.target.value)} /></div>
