...
function OllamaModels() {
  ...
  const have = new Set((d.installed ?? []).map((m: any) => m.name)); const running = new Set((d.running ?? []).map((m: any) => m.name));
  return <div className="stack">
    ...
    <Card title={t("llm_recommended")}><Table cols={[{ k: "name", label: t("name"), render: (r) => <b>{r.name}</b> }, { k: "role", label: "", render: (r) => t("llm_role_" + r.role) }, { k: "note", label: t("note") }, { k: "x", label: "", render: (r) => have.has(r.name) ? <span className="badge green">{t("llm_have")}</span> : <Btn sm onClick={() => pull.mutate(r.name)}>⬇ {t("llm_pull")}</Btn> }]} rows={d.recommended.map((r: any) => ({ id: r.name, ...r }))} />
