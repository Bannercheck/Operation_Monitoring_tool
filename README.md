...
- **Sistem** sayfası (`src/watchover/admin.py`, sol menüde 🛠): sürüm bilgileri (git, motor, Python, Streamlit, çalışma süresi, launcher), arka plan servislerinin durumu (alıcı, kaynak sorgulayıcı, rollup yazımı, canlı öğrenme, simülatör, envanter eşleşmesi), veritabanı tabloları ve boyutu, uygulama günlüğü; **Önbellek** (analiz önbelleği, canlı tampon, oturum durumu, VACUUM); **Güncelleme** (sürüm zip'ini uygulama ya da `git pull`, sonra launcher üzerinden yeniden başlatma; `data/`, `.env`, veritabanları ve `.venv` korunur); **Kullanıcılar** (rol, devre dışı, silme, ekleme); **Giriş** ayarları.
- **Giriş** (`src/watchover/auth.py`): üç mod. `kapalı` (tek operatör), `yerel` (e-posta + parola; scrypt ile karma, ilk hesap yönetici, kendi kendine kayıt ve izinli alan adı kısıtı, roller yönetici / operatör / izleyici, son yönetici korunur), `oidc` (kurumsal SSO: Entra ID, Okta, Keycloak, Google Workspace; Streamlit'in yerel `st.login()` akışı için `.streamlit/secrets.toml` `[auth]` bloğu ayar sayfasından yazılır, `Authlib` gerekir, ilk SSO girişi hesabı oluşturur). Giriş modu açıkken uygulama giriş sayfasıyla korunur, kenar çubuğunda kullanıcı ve çıkış düğmesi bulunur; Sistem sayfası yalnız yöneticilere açıktır. Sınır: yerel modda oturum tarayıcı sekmesi süresince tutulur, sayfa yenilenince yeniden giriş istenir (kalıcı çerez için SSO modu önerilir).
