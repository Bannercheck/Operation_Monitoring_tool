...
| ⚙️ Connection Settings → ITSM | ServiceNow (Table API, basic auth / token, sysparm_query), Jira Service Management (JQL, token), OneDesk, Generic REST (JSON yolu + alan eşleme) veya demo ticket'lar. "Ticket'ları çek" |
...
3. **Canlı (operasyon merkezi):** ajanlar `POST /ingest` ile olay **ve metrik** gönderir (herhangi bir format; metrik için `{"host":..,"cpu":..,"memory":..,"disk":..}` ya da `{"metric":"cpu","value":..}`). Açılış sayfası ve "Canlı" sekmesi 2 saniyede bir güncellenir: **Servis seviyeleri** (erişilebilirlik vs SLO, p95 gecikme, kalan hata bütçesi, SLA durumu), **Altyapı** (host bazlı CPU / bellek / disk, eşik renkleri), olay akışı, **ITSM ticket'ları** (canlı hata sinyalleri, metrik eşik aşımları ve incident'larla ilişkilendirilmiş; ilgililik skoru + gerekçe), son olaylar. "Canlı tamponu analiz et" ile tam pipeline çalışır. Hedefler ve eşikler `scenario.SLO / SLA / METRIC_THRESHOLDS`.
...
| `src/signal_sprint/scenario/__init__.py` | **Hackathon günü dokunulacak tek yer** | CF5.1 | MAPPING, EXTRA_MASKS, EXTRA_DEPENDENCY_WORDS, WINDOW_MIN, WEIGHTS, RECOMMENDATIONS, SLO, SLA, METRIC_THRESHOLDS |
