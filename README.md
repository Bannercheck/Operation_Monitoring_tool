...
| Sayfa | Ne yapar |
|---|---|
| 🏠 Operasyon | Ana sayfa. 2 sn'de bir yenilenir. Başlığın sağındaki 🔎 simgesi Ortam / Sunucu açılır listelerini ve "Filtreyi temizle" düğmesini açar. Asıl gezinme **Ortama göre** kartlarıyla yapılır: her ortam kartı (erişilebilirlik, olay, hata, sunucu sayısı) tıklanabilir; seçilince o ortamın sunucuları ayrı kartlar olarak gelir (CPU, GPU / bellek / disk, olay / hata), bir sunucu seçilince sayfadaki tüm grafikler, servis seviyeleri ve son olaylar o sunucuya daralır. Ekmek kırıntısı "Tümü › staging › worker-01" seçimi gösterir, seçili karta tekrar tıklamak geri çıkar. Sırasıyla: **Altyapı** (host bazlı CPU / GPU / bellek / disk kartları + 15 dk seriler, eşik renkleri), **Servis seviyeleri** (erişilebilirlik vs SLO, p95 gecikme, hata bütçesi, SLA durumu, olay/dk, ERROR+), **olay akışı** (dakika × seviye alan grafiği, en yoğun servisler), **en ilgili 5 ITSM ticket'ı**, **son olaylar** konsolu. Ajan yoksa yerleşik simülatör akar ve SİMÜLASYON rozeti görünür; "Canlı tamponu analiz et" toplanan veriyi tam pipeline'a sokar |
