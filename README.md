...
| Sayfa | Ne yapar |
|---|---|
| 🏠 Operasyon | Ana sayfa. 2 sn'de bir yenilenir. Üstte **filtre çubuğu**: ortam çipleri (prod / staging / dev / qa …) ve tıklanabilir sunucu adları; bir host'a tıklanınca sadece o sunucunun kartı (ortam, son CPU / GPU / bellek / disk, olay, hata, servisler), grafikleri, servis seviyeleri ve son olayları gelir, "Tümü" ile geri dönülür. Sırasıyla: **Altyapı** (host bazlı CPU / GPU / bellek / disk kartları + 15 dk seriler, eşik renkleri), **Servis seviyeleri** (erişilebilirlik vs SLO, p95 gecikme, hata bütçesi, SLA durumu, olay/dk, ERROR+), **Ortama göre** (filtre yokken her ortam için erişilebilirlik, olay, hata, sunucu sayısı kartı), **olay akışı** (dakika × seviye alan grafiği, en yoğun servisler), **en ilgili 5 ITSM ticket'ı**, **son olaylar** konsolu. Ajan yoksa yerleşik simülatör akar ve SİMÜLASYON rozeti görünür; "Canlı tamponu analiz et" toplanan veriyi tam pipeline'a sokar |
