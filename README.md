...
| Sayfa | İçerik |
|---|---|
| 🏠 Operasyon (ana sayfa) | Önce altyapı: host bazlı **CPU / GPU / bellek / disk** kartları ve serileri; sonra servis seviyeleri (erişilebilirlik vs SLO, p95, hata bütçesi, SLA), olay akışı, en ilgili 5 ticket, son olaylar. 2 sn'de bir yenilenir. Yükleme alanı yok |
| 🗄️ Datasets | Dosya / ZIP / TAR.GZ yükleme (1 GB), demo veri seti, canlı tamponu analiz et. Her yüklenen set **ayrı tutulur** (aynı ad tekrar yüklenirse `#2`); **Aktif veri seti** dropdown'ı ile seçilen setin log analizi (Özet, Sinyaller, Incident'lar, Aksiyonlar) açılır; "Kaldır" ile silinir. **Veri setlerini karşılaştır**: A ve B seçilir; temel değerler tablosu (Δ ve Δ %), normalize seviye dağılımı, her setin kendi başlangıcına göre dakika başına olay eğrileri, ikisinde de olan / sadece A / sadece B sinyaller, incident listeleri |
| 🎫 ITSM | Yapılandırma (ServiceNow / Jira Service Management / OneDesk / Generic REST / demo; URL, kullanıcı-şifre veya token, sorgu, alan eşleme), 60 sn otomatik yenileme, ilişkilendirilmiş tam ticket tablosu ve ticket detayı (ilgililik, ilişkili sinyal/metrik/incident, gerekçeler, link) |
| ⚙️ Connection Settings | Veri kaynağı (HTTP API / MCP), Canlı alım (port, API anahtarı, ajan komutları, simülatör), LLM (OpenAI uyumlu base URL, model, anahtar, test) |
| 📖 README | Bu dosya |
...
3. **Canlı (operasyon merkezi):** ajanlar `POST /ingest` ile olay **ve metrik** gönderir (herhangi bir format; metrik için `{"host":..,"cpu":..,"memory":..,"disk":..}` ya da `{"metric":"cpu","value":..}`). Açılış sayfası ve "Canlı" sekmesi 2 saniyede bir güncellenir: **Servis seviyeleri** (erişilebilirlik vs SLO, p95 gecikme, kalan hata bütçesi, SLA durumu), **Altyapı** (host bazlı CPU / GPU / bellek / disk, eşik renkleri; GPU `nvidia-smi` varsa ajan tarafından gönderilir), olay akışı, **ITSM ticket'ları** (canlı hata sinyalleri, metrik eşik aşımları ve incident'larla ilişkilendirilmiş; ilgililik skoru + gerekçe), son olaylar. "Canlı tamponu analiz et" ile tam pipeline çalışır. Hedefler ve eşikler `scenario.SLO / SLA / METRIC_THRESHOLDS`.
    ...
    python agent.py --url http://<dashboard>:8600/ingest --key SECRET --metrics --interval 10  # bu host'un CPU/bellek/disk'i (psutil gerekmez)
...
| `src/signal_sprint/live.py` | Canlı alım: `LiveStore` (olay halka tamponu + metrik tamponu + JSONL spool), HTTP alıcı (`POST /ingest`, X-API-Key / Bearer, `GET /health`), `simulate_batch`, `simulate_metrics`, `start_simulator` | CF5.1 | Gelen gövde aynı parser'lardan geçer; `metric_of` cpu/memory/disk kayıtlarını ayırır; `stats()` dakika × seviye matrisi; `metric_stats()` host bazlı son değer, dakikalık seri, eşik aşımları; `slo()` erişilebilirlik, mesajlardaki `<n>ms` ile p95, hata bütçesi, SLO/SLA durumu; `to_dataset()` tamponu JSONL yapar |
| `src/signal_sprint/itsm.py` | ITSM entegrasyonu ve ticket ↔ metrik/sinyal ilişkilendirme | CF5.1 | `fetch_servicenow` (Table API), `fetch_jira` (REST v3, ADF açıklama), `fetch_onedesk`, `fetch_generic` (JSON yolu + alan eşleme), `demo_tickets`. `correlate`: ticket metnindeki servis/host/anahtar kelimeler ile canlı hata sinyalleri, metrik eşik aşımları ve incident'lar eşleşir (0.65 ağırlık), açılış zamanı bir patlamaya ±30 dk yakınsa zaman skoru (0.35); ilgililik 0..1 + gerekçe listesi |
| `agent.py` | Ajan CLI: `--tail` (log takibi), `--file` (tek seferlik), `--simulate`, `--metrics` (host CPU/bellek/disk: loadavg, /proc/meminfo, disk_usage) | CF5.1 | Stdlib; X-Agent ve X-File-Name başlıkları |
...
| `src/signal_sprint/compare.py` | İki analiz edilmiş veri setini karşılaştırır | CF5.1 | `kpis()` temel değerler; `compare()` KPI deltaları, seviye dağılımı, şablon bazlı ortak / sadece A / sadece B sinyaller (sayı ve patlama farkı), incident listeleri, göreli dakika zaman çizgisi |
...
| `src/signal_sprint/scenario/__init__.py` | **Hackathon günü dokunulacak tek yer** | CF5.1 | MAPPING, EXTRA_MASKS, EXTRA_DEPENDENCY_WORDS, WINDOW_MIN, WEIGHTS, RECOMMENDATIONS, SLO, SLA, METRIC_THRESHOLDS (cpu/gpu/memory/disk) |
...
| `tests/test_itsm_metrics.py` | 3 test: sahte ServiceNow / Jira / genel REST sunucularıyla fetch, ticket ilişkilendirme sıralaması, metrik alımı + eşik aşımı + SLO hesabı | CF5.1 | |
