...
## 1. Kurulum ve çalıştırma
    ...
    python -m pytest -q                    # 25 test, ağ gerektirmez
...
Ortam değişkenleri: `ACTIONS_DB` (aksiyon SQLite yolu), `LIVE_SPOOL` (canlı olay JSONL spool'u), `LIVE_PORT` (alıcı portu).
...
| Sinyaller | Filtreler (seviye, patlama, anlamlı, servis), progress sütunlu tablo, **NEDEN BU SİNYAL?** paneli: gerekçe, şablon, gruplama, patlama detayı, sinyalin dakika grafiği, kanıt tablosu |
| Incident'lar | Kök neden kartı, yayılım Gantt'ı, skor faktör grafiği, zaman çizgisi, korelasyon bağları, ham kanıt satırı, önerilerden tek tıkla aksiyon, özel aksiyon formu, postmortem indir, LLM prompt popover'ı, **🤖 LLM ile açıkla** |
| Aksiyonlar | Öncelik renkli kanban: open / in_progress / done / suppressed, durum değiştirme, silme |
...
| `samples/make_demo.py` | CF5.1 | Sentetik demo veri seti üreticisi | `samples/demo_mixed.zip`: 914 olay, 4 dosya (JSONL, düz log, syslog, CSV); 45 dk arka plan trafiği; gömülü zincir DB gecikmesi → payment timeout → checkout 500 → alarm; ayrı disk dolu incident'ı |

### Motor (`src/signal_sprint/`)

| Dosya | AI aracı | Ne yapar | Nasıl çalışır (fonksiyonlar) |
|---|---|---|---|
| `models.py` | CF5.1 | Kanonik veri modeli | `Observation` (timestamp, message, kind, severity, service, host, attributes, source, line_no, parser, parser_confidence, template, fingerprint; `ref` = `dosya:satır`), `Signal` (`evidence`, `why()` kanıt + gerekçe + güven), `Factor` (ağırlık, değer, katkı, ham veri), `Incident` (kök neden, skor, faktörler, kanıt, zaman çizgisi, bağlar, öneriler, gerekçe kodları), `Action`. `SEV_RANK` seviye sırası |
...
| `parsers/common.py` | CF5.1 | Kayıt → Observation | `records_to_observations()` rolleri ilk 50 kaydın anahtar birleşiminden çözer, zamansız satırları işaretler, dosya adına göre kind (alert / incident), rol dışı alanları `attributes`'a koyar |
...
| Test | pytest + `streamlit.testing.v1.AppTest` + in-process sahte HTTP sunucular | 25 test, ağ gerektirmez |
