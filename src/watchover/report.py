...
T = {
    "tr": {"title": "Servis seviyesi raporu", "period": "Dönem", "scope": "Kapsam", "all": "tüm ortamlar ve sunucular", "generated": "Oluşturulma",
           ...
           "s_err": "{errors} hata / {total} olay; hata bütçesinin %{used} kadarı tüketildi, {breach} dakika SLO altında kaldı.",
    ...
    "en": {"title": "Service level report", "period": "Period", "scope": "Scope", "all": "all environments and hosts", "generated": "Generated",
           ...
           "s_err": "{errors} errors / {total} events; {used}% of the error budget is used, {breach} minutes were below the SLO.",
...
def _svg_chart(per_minute: list[dict], target: float, w: int = 900, h: int = 180) -> str:
    ...
    for i, r in enumerate(per_minute):
        ...
        if r["availability"] is not None:
            ...
            parts.append(f'<rect x="{x + 1:.1f}" y="{top:.1f}" width="{max(bw - 2, 1):.1f}" height="{h - pad - top:.1f}" fill="{col}" opacity="0.85"><title>{r["minute"].strftime("%H:%M")} · {_pct(r["availability"])} · {r["errors"]}/{r["total"]}</title></rect>')
        if r["budget_left"] is not None:
            pts.append(f"{x + bw / 2:.1f},{pad + (h - 2 * pad) * (1 - r['budget_left']):.1f}")
        if i % max(1, n // 8) == 0:
            parts.append(f'<text x="{x + bw / 2:.1f}" y="{h - 8}" text-anchor="middle" fill="#64748b">{r["minute"].strftime("%H:%M")}</text>')
    if pts:
...
def slo_report(ls, analysis=None, env: str | None = None, host=None, window_min: int = 60, lang: str = "tr", org: str = "", logo_svg: str = "") -> str:
    """HTML report for the scope (env, host or hosts) over the last window_min minutes."""
    L = T.get(lang, T["tr"])
    ...
    a = slo["availability"]
    scope = " · ".join(x for x in (env, ", ".join(host) if isinstance(host, (list, tuple)) else host) if x) or L["all"]
    ...
    if a is None:
        ...
        sents.append(L["s_err"].format(errors=slo["errors"], total=slo["total"], used=used if used is not None else "-", breach=det["breach_minutes"]))
    ...
    tiles = [
        ...
        tile(L["events"], f'{slo["total"]:,}', f'{slo["errors"]} {L["errors"]} · {det["breach_minutes"]} {L["breach"]}', "#60a5fa"),
    ]
    ...
    """
    period = f'{(now.replace(microsecond=0) - __import__("datetime").timedelta(minutes=window_min)).strftime("%Y-%m-%d %H:%M")} → {now.strftime("%H:%M")} UTC ({window_min} {L["min"]})'
...
<h2>{L["chart"]}</h2>{_svg_chart(det["per_minute"], tgt)}
<div class="grid2"><div><h2>{L["by_service"]}</h2>{by_service}</div><div><h2>{L["by_host"]}</h2>{by_host}</div></div>
<div class="grid2"><div><h2>{L["latency"]}</h2>{latency}</div><div><h2>{L["templates"]}</h2>{templates}</div></div>
